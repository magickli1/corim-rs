...
impl OpensslSigner {
    ...
    pub(crate) fn public_key_from_x509(cert: &X509Ref) -> Result<Self, CorimError> {
        ...
        let ec_key = pkey
            .ec_key()
            .map_err(|_| CorimError::X5chainUnsupportedKeyType("not an EC2 key".into()))?;
        let group = ec_key.group();
        ...
        let crv = match group.curve_name() {
            ...
            Some(Nid::SECP384R1) => Ok(CoseEllipticCurve::P384),
            Some(Nid::SECP521R1) => Ok(CoseEllipticCurve::P521),
            Some(other) => Err(CorimError::X5chainUnsupportedKeyType(format!(
                "unsupported EC curve {}",
                other.short_name()?
            ))),
            None => Err(CorimError::X5chainUnsupportedKeyType(
...
pub(crate) fn advance_pem_block(data: &[u8]) -> Result<&[u8], CorimError> {
    ...
    let consumed = end_start + end_marker.len() + footer_end + footer.len();
