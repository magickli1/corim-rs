...
//! reloaded on every verify call (Go `crypto/x509` caches its system pool after the
//! first load; this implementation does not cache, so OS trust-store changes take
...
//!   key types are rejected with [`CorimError::X5chainUnsupportedKeyType`].
...
use crate::openssl::{
    ...
    der_tlv_total_len, pem_block_label, validate_signing_certificate, AnchorLoader, OpensslSigner,
};
...
///
/// **System trust backend:** uses [`rustls-native-certs`] (platform PEM bundles), not Go
/// `x509.SystemCertPool`. The store is reloaded on every verify call (Go caches after first
/// load). When the OS trust store is used, certificates that cannot be parsed as X.509 are
...
fn crls_from_der_or_pem(data: &[u8]) -> Result<Vec<X509Crl>, X5chainCrlLoadErrorKind> {
    ...
    if !data.starts_with(b"-----BEGIN") {
        let crl = X509Crl::from_der(data)
            .map_err(|e| X5chainCrlLoadErrorKind::Parse(e.to_string()))?;
        return Ok(vec![crl]);
    }
    ...
    while !rest.is_empty() {
        ...
        if rest.is_empty() || !rest.starts_with(b"-----BEGIN") {
            break;
        }
        let label = pem_block_label(rest)
            .map_err(|e| X5chainCrlLoadErrorKind::Parse(e.to_string()))?;
        if label != "X509 CRL" {
            return Err(X5chainCrlLoadErrorKind::InvalidPemBlockType);
        }
        let crl = X509Crl::from_pem(rest)
            .map_err(|e| X5chainCrlLoadErrorKind::Parse(e.to_string()))?;
        crls.push(crl);
...
fn append_trust_anchors_from_der_or_pem(
    ...
    let der = certificate_der_from_pem_or_der(data)?;
...
/// certificate. PEM CRL files may contain multiple `X509 CRL` blocks; DER CRL files
/// hold a single CRL.
    ...
    F: Fn(&str) -> Result<Vec<u8>, CorimError>,
{
    ...
    for path in crl_paths {
        ...
        let loaded = crls_from_der_or_pem(&data)
            .map_err(|kind| CorimError::X5chainCrlLoadError {
                path: path.to_string(),
                kind,
            })?;
        crls.extend(loaded);
...
fn load_system_trust_anchors() -> Result<Vec<X509>, CorimError> {
    ...
    // Unparseable entries in `native.certs` are omitted from `trust_anchors`; see `TrustAnchors` docs.
    let _ = skipped;
...
/// Parse a single X.509 certificate from DER or PEM (`CERTIFICATE` block); returns DER bytes.
        ...
        None => SystemTime::now()
            ...
            .and_then(|duration| {
                i64::try_from(duration.as_secs()).map_err(|_| {
                    CorimError::custom("system time exceeds i64 seconds range")
                })
            }),
    }
}
...
fn verify_pkix_with_trust_anchors(
    ...
    // ANY skips EKU/purpose enforcement, matching Go `crypto/x509` which does not check
    // extended key usage unless `VerifyOptions.KeyUsages` is set. Leaf signing-key policy
    // is enforced separately by `validate_signing_certificate` before PKIX.
    verify_params
...
fn pkix_failure_error(failure: &PkixFailure) -> CorimError {
    match failure {
        PkixFailure::VerifyError(result) => match result.as_raw() {
            ...
            ffi::X509_V_ERR_CERT_NOT_YET_VALID => CorimError::X5chainCertificateNotYetValid,
...
fn verify_pkix_chain(
    ...
    let result = match &anchors.anchors {
        None => {
            let system_trust_anchors = load_system_trust_anchors()
                .map_err(|e| CorimError::X5chainVerificationFailed(e.to_string()))?;
...
fn check_crl_validity(crl: &X509CrlRef, now_secs: i64) -> Result<(), CorimError> {
    ...
    // A CRL may omit nextUpdate (deprecated by RFC 5280 but permitted); such a CRL is
    // treated as never expiring. lastUpdate is always required and checked above.
...
mod tests {
    ...
    fn assert_x5chain_verification_failed_contains(err: CorimError, needle: &str) {
        match err {
            CorimError::X5chainVerificationFailed(reason) => {
                assert!(
    ...
    fn verify_x509_chain_bad_order() {
        let err = verify_x509_chain(&chain_ders(&["root.cert.pem", "int.cert.pem"]))
            .unwrap_err();
        assert!(matches!(
            err,
    ...
    fn verify_with_x5chain_wrong_explicit_anchor_fails() {
        ...
        let err = verify_with_x5chain(&chain, &anchors).unwrap_err();
        assert_x5chain_verification_failed_contains(err, "unable to get issuer certificate");
    }
    ...
    fn load_trust_anchors_invalid_crl_parse() {
        ...
        assert!(
            matches!(
                err,
                CorimError::X5chainCrlLoadError { ref path, .. } if path == "bad.crl"
            ),
            "unexpected error: {err}"
        );
    }
    ...
    fn signed_corim_verify_with_x5chain_rejects_rsa_leaf_key_type() {
        ...
        let err = signed.verify_with_x5chain(&anchors).unwrap_err();
        assert!(
            matches!(err, CorimError::X5chainUnsupportedKeyType(_)),
            "unexpected error: {err}"
        );
    }
    ...
    fn x5chain_flat_certificate_ders_rejects_oversized_intermediate_concat() {
        ...
        let err = x5chain_flat_certificate_ders(&x5chain).unwrap_err();
        assert_custom_error_contains(err, "exceeds 262144 byte limit");
    }
    ...
    fn parse_concatenated_certificate_ders_rejects_malformed_tlv_length() {
        ...
        assert_custom_error_contains(err, "TLV length exceeds remaining input");
    }
}
